...
class NewspaperBlogDesign1 extends StatelessWidget {
  ...
  final routerContext = context; // Store context
    return Scaffold(
    ...
    for (final item in items) {
      switch (item.type) {
        ...
        case ContentType.featureBox:
          widgets.add(_FeatureBox(text: item.value));
          break;
...
// ── Table ────────────────────────────────────────────────────
class _NpTable extends StatefulWidget {
  ...
  @override
  State<_NpTable> createState() => _NpTableState();
}

class _NpTableState extends State<_NpTable> {
              ...
              child: Column(
                children: [
                  ...
                  ...widget.rows.asMap().entries.map((entry) {
                    ...
                    return Container(
                      color: index.isEven ? Colors.white : _T.paper,
                      child: Row(
                        crossAxisAlignment: CrossAxisAlignment.start,
  ...
  Widget _cell(
    ...
    return SizedBox(
      width: 140, // ✅ fixed readable width
      child: Container(
        ...
        child: Text(
          ...
          style: isHeader
              ? _T.label(11, color: Colors.white)
              : _T.body(13),
        ),
      ),
    );
...
class _FeatureBoxState extends State<_FeatureBox>
  ...
  Widget build(BuildContext context) {
    ...
    final displayTitle =
        widget.customTitle ?? title ?? "Feature Information";
        ...
        child: Column(
          children: [
            ...
            AnimatedBuilder(
              ...
              builder: (_, __) {
                return SizedBox(height: _controller.value * 8);
              },
            ),
          ],
        ),
  ...
  /// 🔥 Smart content parser (better than plain RichText)
  List<Widget> _buildContent(String content) {
    ...
    return lines.map((line) {
      ...
      /// 🔷 TABLE DETECT (skip for now or render custom)
      if (line.startsWith('|')) {
        return Padding(
          padding: const EdgeInsets.symmetric(vertical: 6),
          child: Text(
            line,
            style: _T.body(13).copyWith(
      ...
      return Padding(
        ...
        child: Text(
          line,
          style: _T.body(14).copyWith(height: 1.5),
        ),
      );
    }).toList();
  }
